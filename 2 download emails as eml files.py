...
def main():
    ...
    if not all_message_in_label:
        ...
        for emails in all_message_in_label: 
            ...
            # ----------------    folder = threadID (that will be converted in subject at the end of the script) and filename =  date (converted into " 2017_02_04 15h45m32")
            ...
            #retrieve headers containing date and time
            ...
            try:
                ...
                emlfile = os.path.join(path_to_folder, f'{date_as_filename}.eml')
    ...
    #  rename the folder thread ID into the subject of the first email in the thread

    # loop across the folder name to rename them
    for root, dirs, files in os.walk(folder_to_save_all_emails):
        for dir in dirs:
            print(dir)

            response= service.users().threads().get(userId="me", id=dir, metadataHeaders=None, format="full").execute()

            # get headers of the first message 
            headers=response["messages"][0]["payload"]["headers"]
            # print(headers)
            #  the subject of first message 
            ...
            # get the date of first message
            date= [i['value'] for i in headers if i["name"]=="Date"]
            date_as_valid_filename= valid_path_name(date[0])
            date_as_foldername= convert_date(date_as_valid_filename, False)
            print(date_as_foldername)


            new_foldername= date_as_foldername + "  " + subject_as_foldername

            print(root+ "\\"+ dir)
            print(root+ "\\"+ new_foldername)
            shutil.move(root+ "\\"+ dir, root+ "\\"+ new_foldername)
            print("folder renamed: ", new_foldername)
